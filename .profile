...
function parse_vcap_services () {
  ...
  export OPENAI_API_BASE_URLS=$(    echo "$VCAP_SERVICES" | jq -r '[.genai[].credentials |  select(.model_capabilities[]=="chat") | .api_base] | join(";")')
  export OPENAI_API_KEYS=$(         echo "$VCAP_SERVICES" | jq -r '[.genai[].credentials |  select(.model_capabilities[]=="chat") | .api_key] | join(";")')
